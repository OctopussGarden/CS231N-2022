...
def rnn_step_backward(dnext_h, cache):
    ...
    dnext_h = np.multiply(dnext_h, 1-next_h**2) # (N, H) 2nd item is the tanh deriative of next_h
    dx = dnext_h @ Wx.T # (N, D)
...
def rnn_backward(dh, cache):
    ...
    dx, dprevh = np.zeros((N,T,D)),np.zeros((N,H))

    # Loop to update gradients
    for t in reversed(range(T)):
        dh_copy[:,t,:] += dprevh
        dx[:,t,:], dprevh, temp_dWx, temp_dWh, temp_db = rnn_step_backward(dh_copy[:,t,:], cache[t])
        dWx += temp_dWx
        dWh += temp_dWh
        db += temp_db
    ...
    # # Loop to update gradients
    # for t in reversed(range(T)):
    #     dx[:,t,:], dprevh, temp_dWx, temp_dWh, temp_db = rnn_step_backward(dh[:,t,:]+dprevh, cache[t])
    #     dWx += temp_dWx
    #     dWh += temp_dWh
    #     db += temp_db
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    # Concatenate matrices for computational convience
    x_con = np.concatenate((prev_h, x), axis=1) # (N, H+D)
    W_con = np.concatenate((Wh, Wx), axis=0) # (H+D, 4H)

    score_matrix = x_con @ W_con + b # (N, 4H)

    # Define gates
    gates = sigmoid(score_matrix[:, :3*H])
    i, f, o = gates[:, :H], gates[:, H:2*H], gates[:, -H:]  # (N, H)
    g = np.tanh(score_matrix[:, -H:]) # (N, H)

    next_c = i * g + f * prev_c # (N, H)

    tanh_nextc = np.tanh(next_c) 
    next_h = o * tanh_nextc # (N, H)

    cache = (prev_c, x_con, W_con, gates,  g, tanh_nextc)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    prev_c, x_con, W_con, gates, g, tanh_nextc = cache
    Wh, Wx = W_con[:H, :], W_con[H:, :]  # (H, 4H) (D, 4H)
    i, f, o = gates[:, :H], gates[:, H:2*H], gates[:, -H:]  # (N, H)

    # gates deriatives
    dgates = np.multiply(gates, 1.-gates) # ([[d_i], [d_f], [d_o]]) ---> (N, 3H)
    d_g = 1. - g**2 # (N, H)

    # TODO: Added
    dnext_c += dnext_h * (1-tanh_nextc**2) * o  # (N, H)

    # Deriatiaves calculation
    dO = np.multiply(dnext_h, tanh_nextc)
    dF = np.multiply(dnext_c, prev_c)
    dI = np.multiply(dnext_c, g)
    dG = np.multiply(dnext_c, i) # (N, H)
    dIFO = np.concatenate((dI, dF, dO), axis=1) # (N, 3H)

    # multiply tanh and sigmoid derivatives
    dIFO = np.multiply(dIFO, dgates)
    dG = np.multiply(dG, d_g)

    # Auxiliary matrix
    dA_con = np.concatenate((dIFO, dG), axis=1) # (N, 4H)

    dWhx = x_con.T @ dA_con # (H+D, 4H) = (H+D, N) * (N, 4H)
    dWh, dWx = dWhx[:H, :], dWhx[H:, :] # (H, 4H) (D, 4H)

    dx = dA_con @ Wx.T # (N, D) = (N, 4H) * (4H, D)
    
    db = np.sum(dA_con, axis=0)  # (4H,)

    dprev_h = dA_con @ Wh.T  # (N, H) = (N, 4H) * (4H, H)
...
def lstm_backward(dh, cache):
    ...
    N, T, H = dh.shape
    D = cache[0][1].shape[1] - H # x_con.shape = (N, H+D)

    dWx, dWh, db = np.zeros((D, 4*H)), np.zeros((H, 4*H)), np.zeros((4*H,))
    dx, dprevh, dprevc = np.zeros((N, T, D)), np.zeros((N, H)), np.zeros((N, H))
    ...
    for t in reversed(range(T)):
        dh_copy[:, t, :] += dprevh
        dc[:, t, :] += dprevc
        dx[:, t, :], dprevh, dprevc, temp_dWx, temp_dWh, temp_db = lstm_step_backward(dh_copy[:, t, :], dc[:, t, :], cache[t])
        dWx += temp_dWx
        dWh += temp_dWh
        db += temp_db
