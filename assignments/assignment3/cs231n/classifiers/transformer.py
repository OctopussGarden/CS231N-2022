...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        # Embed captions and projecet images
        seq = self.embedding(captions) # (N, T, W)
        seq = self.positional_encoding(seq) # (N, T, W)
        img = self.visual_projection(features).unsqueeze(1) # (N, W) ---> (N, T, W)

        # Get output mask
        tgt_mask = torch.tril(torch.ones(T, T))

        # Apply decoder        scores = self.transformer(tgt_mask=tgt_mask, tgt=seq, memory=img)  # (N, T, W)
