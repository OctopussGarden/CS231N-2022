...
def svm_loss(x, y):
    ...
    y_hat_true = x[range(N), y][:, np.newaxis]    # scores for true labels
    margins = np.maximum(0, x - y_hat_true + 1)   # margin for each score
    loss = margins.sum() / N - 1  # regularized loss

    dx = (margins > 0).astype(float)    # initial gradient with respect to Y_hat (i.e. x)
    # update gradient to include correct labels
    dx[range(N), y] -= dx.sum(axis=1)
    dx /= N      # gradient with respect to x
